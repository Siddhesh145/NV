var detailinfopageController = function ($scope, $location, commonService, globals, ngMeta, $rootScope) {
...
$scope.onCategoryChange = function (val) {
	...
	{
		if ($scope.errormsg == "Please Enter following : " && $scope.productid != 2) { $scope.errormsg += "Last Name" } else { $scope.errormsg += " ,Last Name"
	};
	}
	...
	{
		if ($scope.errormsg == "Please Enter following : ") { $scope.errormsg += "Gender" } else { $scope.errormsg += " ,Gender"
	};
	}
	...
	if ($scope.errormsg == "Please Enter following : ")
	{
		...
		if (globals.ValidateForm(Date1)) {
		    //alert("Valid");
		    if (diff > 216 && $scope.productid == 1)
		    {
				console.log('invalid');
				$scope.errormsg = "Sorry, this form is for ordering a new name for a Child less than 18 years old. Please visit the Adult Name section for any other new name or correction request.";
				return false
		    }
		    else if (diff > 216 && $scope.productid == 8)
		    {
				console.log('invalid');
				$scope.errormsg = "Sorry, this form is for ordering a new name for a Child less than 18 years old. Please visit the Adult Name section for any other new name or correction request.";
				return false
		    }
		    else if (diff < 0 && $scope.productid == 8)
		    {
				console.log('invalid');
				$scope.errormsg = "Sorry, Date of Birth Entered is future date.Please check Date of Birth again.";
				return false
		    }
		    else if (diff < 0 && $scope.productid == 1)
		    {
				console.log('invalid');
				$scope.errormsg = "Sorry, Date of Birth Entered is future date.Please check Date of Birth again.";
				return false
...
detailinfopageController.$inject = ['$scope', '$location', 'commonService', 'globals', 'ngMeta', '$rootScope'];